...
- **원격 데스크톱은 VPN 안쪽에서만.** Pro는 RDP 호스트가 켜지므로, 공유기에서 3389를 열지 않는다. 밖에서 화면이 필요하면 터널이 붙은 뒤 LAN 주소로 붙는다. Chrome 원격 데스크톱·AnyDesk는 아웃바운드 중계라, **이 PC를 VPN 클라이언트로 전체 터널하지 않으면** Server만 켜 둬도 같이 쓰는 경우가 많다.
