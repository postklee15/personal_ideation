# 미국 30년물 금리 · AI 자본 전쟁 브리핑

- 날짜: 2026-08-18
- 상태: active
- 한 줄: 네러티브는 분자를 바꾸고, 높은 실질금리는 분모로 자산의 오늘 값을 정한다

후속 논의에서 규모·반론·한국 렌즈를 대입했고, 일본 국채·엔캐리와 시스템 전이 관찰 구간을 붙였다. 베선트 바이백과 워시 연준이 장기채를 과거처럼 흡수할 수 있는지를 같은 기능/가격 틀로 적었다. 8월 19일 트위스트 헤드라인에 금·은·비트코인이 뛴 경로도 분모와 네러티브로 갈랐다. 개인투자자 대응의 순서를 가계 언어로 옮긴 뒤, 같은 필터를 가상화폐 시장에 대입했다.

합본 (브리핑 · 후속 논의 · 가상화폐 · 개인 대응)

- `us-30y-ai-capital-combined.typ` — 네 편을 한 권으로 묶은 Typst
- `미국_30년물_금리_AI_자본전쟁_합본.pdf` — 열람용 합본 PDF
- `parts/` — 네 편의 공유 본문. 단독 원본도 여기를 include 한다.

선행 브리핑

- `미국_30년물_금리_AI_자본전쟁_브리핑.pdf` — 열람용 PDF
- `us-30y-ai-capital-briefing.typ` — Typst 원본

후속 논의

- `discussion-who-blinks.md` — 논의용 마크다운
- `누가_먼저_항복하는가_후속논의.pdf` — 열람용 PDF
- `discussion-who-blinks.typ` — Typst 원본

가상화폐 렌즈

- `crypto-unproductive-duration.md` — 논의용 마크다운
- `가상화폐_생산성없는_자산의_시간.pdf` — 열람용 PDF
- `crypto-unproductive-duration.typ` — Typst 원본

개인투자자 대응 틀

- `individual-investor-playbook.md` — 논의용 마크다운
- `개인투자자_대응틀.pdf` — 열람용 PDF
- `individual-investor-playbook.typ` — Typst 원본

- `build.sh` — 로컬 재빌드 스크립트

작성 기준일은 2026년 8월 18일이다. 투자 권유가 아니다.
